...
try:
   while True:
      ...
      if id == "l":
              ...
              if right < 0:
                right = 0
              print "left %s    right %s throttle %s" % ( left, right, throttle)
              pi.hardware_PWM(12, 1000, left*1000000)              
              pi.hardware_PWM(13, 1000, right*1000000)
              ...
              if (ardIn>=0) & (ardIn<=207):
                ...
                ardOut = 48
              print ardOut
              ser.write(str(chr(ardOut))) # Convert the decimal number to ASCII then send it to the Arduino
              print ser.readline() # Read the newest ardOut from the Arduino
      elif id == "y":
