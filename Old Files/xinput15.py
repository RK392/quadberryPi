...
def handle_output(conn, addr, signal_list):
    """
...
def handle_input(conn, addr, signal_list):
    try:
    ...
    try:
        ...
        while(True):
            try:
                ...
                t1 = threading.Thread(name="handle_input_"+str(c_count), target=handle_input, args=(conn, addr, signal_list))
                t2 = threading.Thread(name="handle_output_"+str(c_count), target=handle_output, args=(conn, addr, signal_list))
                t1.start()
                t2.start()
                threads.append(t1)
                threads.append(t2)
